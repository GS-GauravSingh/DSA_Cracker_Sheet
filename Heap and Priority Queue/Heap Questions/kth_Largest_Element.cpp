...
using namespace std;
/*
Approach 1:- sort the given array and return the (n - k)th index.
...
Step 1:- create min heap for first k elements.
...
*/
int kthLargestElement_Using_Heap(vector<int> &arr, int k)
{
    // Base Case
    if (arr.size() == 0)
    {
    ...
    */
    ...
    // Step 3:- after step 2, our heap will contain the k largest elements and the element present at heap.top() is the kth largest element.
    int ans = pq.top();
    return ans;
}
...
int main()
{
    // from the given unsorted array, your task is to return the kth largest element.
    vector<int> arr = {7, 10, 4, 3, 20, 15};
    int k = 4;

    // int ans = kthLargestElement(arr, k);
    int ans = kthLargestElement_Using_Heap(arr, k);
    if (ans != -1)
    {
