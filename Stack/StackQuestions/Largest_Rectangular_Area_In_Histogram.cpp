...
you can extend the width of current bar towards left and right only when the bars present at left and right having heights greater than or equal to the height of current bar.

Extend = left bars height >= current bar height <= right bars height.

Jaise hi appko left or right me pheli bar milegi jiski height current bar ki height se choti hai, vaha pe aapko current bar ka extension rokna padeha.

So, isi cheez ko dhayan rkthe hue,
Step:-
...
int main(void)
{
    ...
    cout << largest_rectangle_in_Histogram(arr);
}