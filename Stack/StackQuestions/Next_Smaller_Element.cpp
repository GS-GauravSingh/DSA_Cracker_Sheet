...
vector<int> nextSmallerElementUsingStack(vector<int>& arr){
    ...
    for (int i = arr.size()-1; i >= 0; i--)
    {
        ...
        // if s.top() > arr[i], then iterate through the whole stack and find if there is any element which is less than arr[i], if yes then our loop terminates at that element, else out loop terminates at -1, because initially we push -1 into stack.
        else{
...
int main(){
    ...
    cout<<"Answer using brute force approach.\n";
    ans = nextSmallerElement(arr);
    printVec(ans);


    cout<<"Answer using Optimized approach.\n";
